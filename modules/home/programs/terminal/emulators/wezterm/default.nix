  ...
  config = mkIf cfg.enable {
    programs.wezterm = {
      ...
      extraConfig = ''
        ...
        -- Follow the DMS matugen palette when present: the
        -- colors/dank-theme.toml scheme is (re)generated by matugen on every
        -- theme change. Fall back to a static scheme if it hasn't been rendered
        -- yet (e.g. a fresh machine before the first DMS theme run).
        local color_scheme = 'Kanagawa (Gogh)'
        do
          local f = io.open(wezterm.home_dir .. '/.config/wezterm/colors/dank-theme.toml', 'r')
          if f ~= nil then
            f:close()
        ...
        return {
          ...
          -- Color scheme (matugen "dank-theme" when available, else fallback)
