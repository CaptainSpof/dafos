...
let
  ...
  niriRingsTemplatePath = "${matugenConfigDir}/templates/niri-rings.kdl";
  niriRingsOutPath = "${config.xdg.configHome}/niri/dms/dafos-rings.kdl";
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    xdg.configFile."matugen/templates/niri-rings.kdl".text = ''
      // Generated by matugen — dynamic niri ring colours (see dms/default.nix).
      layout {
          focus-ring {
              width 2
              active-gradient from="{{colors.primary.default.hex}}" to="{{colors.inverse_primary.default.hex}}" angle=45 in="oklch shorter hue"
              inactive-color "transparent"
          }

          border {
              width 1
              active-color   "{{colors.surface_container_lowest.default.hex}}"
              inactive-color "{{colors.surface_container_lowest.default.hex}}"
          }
      }
    '';
    ...
    xdg.configFile."matugen/config.toml".text = lib.mkForce ''
      ...
      [templates.niri-rings]
      input_path = "${niriRingsTemplatePath}"
      output_path = "${niriRingsOutPath}"
    '';
    ...
    programs.dank-material-shell = {
      ...
      niri = {
        ...
        # Order matters: niri lets the *last* include win, and DMS's own
        # colors.kdl sets a flat active-color on both rings. "dafos-rings" is
        # the matugen-rendered override defined above (it lands in the same
        # niri/dms/ directory purely because that is the only path shape this
        # option can emit) and must therefore stay last in this list.
